...
function fileInput() {
    $("input[type='file']").each(function() {
        ...
        if (!$(this).parents(".input-group-addon").length) {
            var chooseLabel = $(this).data("choose") || "Выбрать";
            var removeLabel = $(this).data("remove") || "Удалить";
            var changeLabel = $(this).data("change") || "Изменить";

            var template = "<div class='fileinput fileinput-new input-group' data-provides='fileinput'>" +
                "<div class='form-control' data-trigger='fileinput'>" +
                "<i class='glyphicon glyphicon-file fileinput-exists'></i> <span class='fileinput-filename'></span></div>" +
                "<span class='input-group-addon btn btn-default btn-file'><span class='fileinput-new'>"+chooseLabel+"</span><span class='fileinput-exists'>"+changeLabel+"</span>" +
                $(this)[0].outerHTML +
                "</span><a href='#' class='input-group-addon btn btn-default fileinput-exists' data-dismiss='fileinput'>"+removeLabel+"</a>";
...
function yiiPjaxFix() {
    ...
    // Переопределяем вызов событий после подтверждения
    ...
    // Переопределяем отображение подтверждения об удалении в виде модального окна
    yii.confirm = function (message, ok, cancel) {
        var modal = $("#confirm-modal");

        $(modal.find(".modal-title")).html("<strong>Удаление</strong>");
        $(modal.find(".modal-body")).html("Вы уверены, что хотите <strong>удалить</strong> этот элемент?");
        $(modal.find("button").unbind().bind("click", function () {

            if ($(this).data("action") == "success")
                !ok || ok();
            else
                !cancel || cancel();
