...
class Generator extends \yii\gii\Generator
{
    ...
    public $modelClass;
    public $modelName;
    public $baseClass = 'app\components\Model';
    ...
    {
        return 'Этот генератор создает модель, наследуя ее от базового класса модели';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'tableName', 'modelClass', 'modelName', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass'], 'filter', 'filter' => 'trim'],
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            ...
            'baseClass' => 'Наследовать от',
            'generateRelations' => 'Генерировать связи',
            'generateLabelsFromComments' => 'Генерировать подписи из комментариев к полям',
            'generateQuery' => 'Генерировать ActiveQuery',
            'queryNs' => 'ActiveQuery Namespace',
            'queryClass' => 'ActiveQuery Class',
            'queryBaseClass' => 'ActiveQuery Base Class',
            'generateScenarios' => 'Генерировать сценарии',
            'useTablePrefix' => 'Префикс таблицы',
            'generateBehaviors' => 'Прикрепить поведения',
        ]);
    }
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            ...
            'modelName' => 'Название модели, которое будет использоваться в CRUD. Название должно быть во <code>множественном числе</code> и в <code>именительном падеже</code>.',
        ]);
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $params = [
                ...
                'attachedBehaviors' => $this->attachedBehaviors,
            ];
            $files[] = new CodeFile(
    ...
    /**
     * Генерирует поведения для разных нужд
     * @param $table \yii\db\TableSchema $table the table schema
    ...
    public function generateBehaviors($table) {
        ...
        foreach ($table->columns as $column) {
            if (preg_match('/^(photo|image|pic)$/i', $column->name)) {
                $files[] = [
                    'name' => $column->name,
                    'type' => 'image'
                ];
        ...
        if (count($files)) {
            $this->attachedBehaviors[]= 'app\components\behaviors\FileBehavior';
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $column) {
            ...
            if ((!$column->allowNull && $column->defaultValue === null) && !preg_match('/^(photo|image|pic)$/i', $column->name)) {
        ...
        foreach($files as $type => $columns) {
            $rule = "[['" . implode("', '", $columns) . "'], '$type' ";
            switch($type) {
                case "image" :
                    $rule.=", 'extensions' => ['jpg','jpeg','png']]";
                    break;
                default:
                    $rule.="]";
            }
    ...
    public function successMessage() {
        return 'Код был успешно сгенерирован';
    }
}
